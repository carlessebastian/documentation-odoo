...
- **Última actualización**: 2026-05-12 (Fase 5.1 — loader 1 partners
  ejecutado + **notas Holded inyectadas en `res.partner.comment` para
  auditoría UI**: 306 partners (150 "Código Holded" + 156 "VAT
  rechazado") con nota visible en Notas internas. Capa 1 build +
  Capa 2 fallback en `_partners_lib.py`; detección por sentinel
  textual (markers HTML comment descartados por sanitización Odoo
  asimétrica). 146 tests offline verdes. Script one-shot
  `retrofit_holded_code_note.py` para partners ya cargados.)
- **Última fase completada**: **Bloque D — Fase 5.1 loader 1 partners
  + notas Holded en UI**.
...
- **Próxima acción**: pre-trabajo para **Fase 5.3**:
  ...
  3. Sesión con operador para rellenar
     ...
     loader 4 (purchases)**.
  4. Escribir loader 2 (products + services consumiendo
     `HoldedResolvers.resolve_tax`). 1.569 products + 440 services
     en dry-run primero.
