...
- **Próxima acción**: pre-trabajo para **Fase 5.3**:
  ...
  9. ~~Loader 9 PDFs invoice~~ ✅ ejecutado 2026-05-12 +
     ~~creditnote~~ ✅ ejecutado 2026-05-13.
     - Invoice: 3.422 ir.attachment (~143 MB).
     - Creditnote: 678 ir.attachment (~33 MB) tras re-dumpear
       `pdfs/creditnote/` (dump original `2026-05-11` se hizo con
       default `--pdf-doc-types invoice,purchase` y se saltó
       creditnote/purchaserefund → pitfall documentado en agent
       memory + runbook Apéndice G).
     - **Cobertura SALES final**: 4.106/4.108 (99,95%), 2
       residuos = smoke test Fase 4.6.
     - PDFs purchase (4.059 / 801 MB) y purchaserefund (69)
       diferidos hasta loader 4. Snapshot
       `2026-05-13_pdfs-creditnote-fix.json`.
...
| 2026-05-13 | Bloque D Fase 5.1 (fix PDFs creditnote — ejecutado) | **Auditoría visual operador detectó facturas SALES sin PDF en chatter**. Query cobertura: 680 de 4.108 SALES sin PDF (16,6%). Causa raíz única: dump `2026-05-11` se generó con `--pdf-doc-types invoice,purchase` (default `holded-export`), faltaba `pdfs/creditnote/`. Loader 9 reportó `0 errors` porque su lógica es "itera lo que haya" sin assertion de cobertura. **Distribución de los 680**: 653 out_refund AC- (loader 5 originales) + 26 out_invoice ACC- (operaciones flip cuyo origen Holded es creditnote, posteadas cierre 5.1 status=3) + 1 smoke test (id=2). **Fix en 4 pasos**: (1) re-dump solo creditnote PDFs: `holded_export.py --resources documents --doc-types creditnote --include-pdfs --pdf-doc-types creditnote --resume --no-confirm` → 678 PDFs descargados (~33 MB), 0 errors. (2) verificar dump: 678/678, 0 zero-byte, 0 huérfanos. (3) dry-run `loader_pdfs.py --doctype creditnote`: 678 would_create, 0 errors. (4) run real: 678 created en 3:04 min, 0 errors. **Cobertura final SALES**: 4.106/4.108 PDFs (99,95%); 2 residuos = smoke test Fase 4.6 (id=2 + su rectificativa id=4), no son reales. **Pitfall documentado cross-tenant**: agent memory `project_holded_dump_pdf_scope.md` ("todo dump Holded debe `--pdf-doc-types invoice,purchase,creditnote,purchaserefund`"), runbook Apéndice G con procedimiento de recovery, runbook §9 con warning, decisions-log entrada nueva. Snapshot `2026-05-13_pdfs-creditnote-fix.json` (totals + residual_no_pdf). `purchaserefund` PDFs diferidos al cutover (mismo dump nuevo, no este congelado). **Próximo paso no cambia**: tax_reclassification.yaml → desbloquea loader 4 purchases. |
